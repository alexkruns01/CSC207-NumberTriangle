...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        NumberTriangle top = new NumberTriangle(Integer.valueOf(line));
        NumberTriangle[] up = new NumberTriangle[]{top};
        NumberTriangle[] bot;
        ...
        while (line != null) {
            ...
            bot = new NumberTriangle[lines.length];

            int i = 0;
            for (String stuff : lines) {
                bot[i] = new NumberTriangle(Integer.valueOf(stuff));
                i++;
            }
            int j = 0;
            while(j < up.length){
                up[j].setLeft(bot[j]);
                up[j].setRight(bot[j + 1]);
                j ++;
            }
